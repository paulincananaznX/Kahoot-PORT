    ...
    const submitAnswer = (optionId: string) => {
        ...
        if (isCorrect) {
            ...
            get(ref(db, `rooms/${roomId}/startTime`)).then((snap) => {
                ...
                get(ref(db, `rooms/${roomId}/players/${playerInfo?.id}/score`)).then((sSnap) => {
                    const currentScore = sSnap.val() || 0;
                    update(ref(db, `rooms/${roomId}/players/${playerInfo?.id}`), {
